...
Table (xterm-compatible): printable → `text` bytes (UTF-8); `Alt`+printable → `ESC` + bytes (macOS Option-as-Alt is a config flag, default off); `Ctrl`+letter → `0x01..0x1A`, plus `Ctrl+[ \ ] ^ _`, `Ctrl+Space` → NUL; `Enter` → `\r`, `Tab` → `\t`, `Shift+Tab` → `ESC[Z`, `Backspace` → DEL (config: BS), `Escape` → `ESC`; arrows → `ESC[A..D`, or `ESC O A..D` under DECCKM (application cursor keys), modified → `ESC[1;{m}A` with `m = 1 + shift·1 + alt·2 + ctrl·4`; Home/End → `ESC[H/F` (`ESC O H/F` in app mode); Insert/Delete/PgUp/PgDn → `ESC[2~ 3~ 5~ 6~` (+`;m`); F1–F4 → `ESC O P..S`, F5–F12 → `ESC[15~ 17~ 18~ 19~ 20~ 21~ 23~ 24~`. `Cmd+Enter` is the newline chord: it is sent as `Alt+Enter` (`ESC CR`) since xterm has no Super encoding and line editors treat Meta+Enter as "insert newline"; every other `Cmd` chord is the app's. Alt always `ESC`-prefixes the keys that produce no character (Enter, Tab, Escape, Backspace), whatever the Option-as-Alt flag says. **Kitty keyboard protocol**: the Server advertises it and reports `Modes::KITTY_KEYBOARD` once an application pushes it; the encoder then applies the "disambiguate escape codes" flag — `Escape` → `ESC[27u`, modified Enter/Tab/Backspace → `ESC[13;{m}u` / `ESC[9;{m}u` / `ESC[127;{m}u`, Ctrl/Alt+letter → `ESC[{codepoint};{m}u` with the unshifted code point; plain Enter/Tab/Backspace, text, arrows and function keys keep their legacy forms.
